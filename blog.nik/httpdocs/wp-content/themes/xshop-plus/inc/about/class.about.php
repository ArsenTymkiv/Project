...
if ( ! class_exists( 'xshop_About' ) ) {
	...
	class xshop_About {
		 ...
		 *
		 * @var xshop_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof xshop_About ) ) {
				self::$instance = new xshop_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap xshop-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/xshop-pro/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo esc_html ('X Shop Pro Live Preview', 'xshop-plus');
					echo '</a></td></tr>';
        ...
        'activating_string'        => __( 'Activating', 'xshop-plus' )
        )
    );
